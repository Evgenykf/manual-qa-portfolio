...
- The "never ordered" half reuses the standard orphan-finding pattern (`LEFT JOIN` + `IS NULL`) from the join practice — in the current dataset every product has been ordered at least once, so this half returns 0 rows, and the combined result above is just the four ordered products.
